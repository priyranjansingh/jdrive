...
class Temp extends BaseModel
{
	...
	public function rules()
	{
		...
		return array(
			array('id, s3_bucket, acl, type, file_name,file_size, s3_url, user_id, created_by, modified_by, date_entered, date_modified', 'required'),
			array('acl, type, status, deleted', 'numerical', 'integerOnly'=>true),
			array('id, user_id, created_by, modified_by', 'length', 'max'=>36),
			array('s3_bucket, file_size', 'length', 'max'=>128),
			array('file_name', 'length', 'max'=>256),
			array('s3_url', 'length', 'max'=>512),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, s3_bucket, acl, type, file_name,file_size, s3_url, user_id, status, deleted, created_by, modified_by, date_entered, date_modified', 'safe', 'on'=>'search'),
		);
	}
